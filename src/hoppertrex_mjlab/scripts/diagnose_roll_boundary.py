...
def summarize_trials(trials: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
  ...
  for height in DIAGNOSTIC_HEIGHTS_M:
    ...
    summaries.append({
      ...
      'maximum_progress_m': max(
        float(row['max_progress_past_face_m']) for row in rows
      ),
    })
  return summaries
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
  ...
  parser.add_argument('--mode', choices=('events', 'posture-grid'), required=True)
...
def main(argv: Sequence[str] | None = None) -> None:
  ...
  payload = (
    run_event_diagnostic(args) if args.mode == 'events' else run_posture_grid(args)
  )
