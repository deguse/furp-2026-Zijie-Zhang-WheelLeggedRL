...
- Hybrid-v3 first machine-room `QualifyTrigger` at `4f22e38` failed closed because a flat-only zero spawn offset leaked into the 1 cm stair session. Repair `c1d9744` restores the canonical stair-approach spawn; 146/146 targeted tests and a 16-env CPU single-riser smoke (left 11/right 8, zero resets) passed. The old campaign remains failure evidence only; formal GPU false-positive gates move to a new sealed campaign. (Codex: 2026-08-12, trigger-spawn repair)
